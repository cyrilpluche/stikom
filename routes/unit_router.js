...
router.put('/update',
    policy.checkParameters(['unit_id', 'unit_name']),
    function (req, res, next) {
        modelUnit.update(req.body.unit_id, req.body.unit_name).then(function (data) {
            if (!data) {
                throw ERRORTYPE.INTERNAL_ERROR
            } else {
                res.json({data: data})
...
router.put('/update_execute',
    policy.checkParameters(['unit_id', 'activity_id']),
    function (req, res, next) {
        modelUnit.updateExecute(req.body.unit_id, req.body.activity_id)
            .then(function (data) {
                if (!data) {
                    throw ERRORTYPE.INTERNAL_ERROR
                } else {
